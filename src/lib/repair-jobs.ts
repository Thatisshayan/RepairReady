 ...
 * mean to place a real call to that person. Configured via VITE_SAFE_DEFAULT_PHONE rather than
 * hardcoded, so a real number never has to sit in public source -- an unset value degrades to no
 * pre-fill (a blank phone field), not a broken one.
 */
const CONFIGURED_SAFE_DEFAULT_PHONE = import.meta.env.VITE_SAFE_DEFAULT_PHONE?.trim() ?? "";
export const SAFE_DEFAULT_PHONE = PHONE_RE.test(CONFIGURED_SAFE_DEFAULT_PHONE)
  ? CONFIGURED_SAFE_DEFAULT_PHONE
  : "";
