...
export interface ReadinessQueueSummary {
  ...
  /** null when no visit outcome has been recorded across any loaded job yet -- an intentional
   * "not enough data" state, never a misleading 0%. */
  firstTimeFix: FirstTimeFixSummary | null;
}
...
export async function loadReadinessQueue(
  ...
  const summary: ReadinessQueueSummary = {
    ...
    firstTimeFix: firstTimeFixRate(items.map((item) => item.brief)),
  };
