  ...
  return (
    <section className="rr-readiness-queue flex h-full min-h-0 flex-col" aria-labelledby="readiness-queue-title">
      <header className="rr-queue-header space-y-3 border-b border-border p-3.5 sm:p-4">
        ...
        <div className="rr-first-time-fix-stat rounded-lg border border-primary/20 bg-primary/5 px-3 py-2.5">
          <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-primary">First-time-fix rate</p>
          {summary.firstTimeFix ? (
            <p className="mt-1 text-sm font-semibold text-foreground">
              {Math.round(summary.firstTimeFix.rate * 100)}% ({summary.firstTimeFix.firstTimeFixes} of{" "}
              {summary.firstTimeFix.recorded}) — repaired on the first visit, no second visit needed
            </p>
          ) : (
            <p className="mt-1 text-sm leading-relaxed text-muted-foreground">
              Not enough data yet. Record a visit outcome on a job's technician brief to start measuring this.
            </p>
          )}
        </div>
