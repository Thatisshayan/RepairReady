...
Deno.test("buildTask includes the recipient name so the agent can greet by name", () => {
  const task = buildTask({}, "Jane Doe");
  assertEquals(task.includes("Recipient name: Jane Doe"), true);
});

Deno.test("buildTask falls back to a generic name when none is saved", () => {
  const task = buildTask({});
  assertEquals(task.includes("Recipient name: the customer"), true);
});

Deno.test("buildTask always discloses recording and preliminary-assessment use", () => {
  const task = buildTask({});
