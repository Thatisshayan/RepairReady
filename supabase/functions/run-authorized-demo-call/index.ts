...
Deno.serve(async (req) => {
  ...
  const demoPhone = Deno.env.get("CALLE_DEMO_PHONE") ?? "";
  if (!CANONICAL_PHONE_RE.test(demoPhone)) {
    console.error("run-authorized-demo-call configuration error", "missing_or_invalid_demo_phone");
    return jsonResponse({ status: "error", message: "The authorized demo recipient is not configured. No call was placed." }, 503, origin);
  }
  ...
  const attemptPayload = { created_by: ownerId, demo_marker: DEMO_MARKER, repair_job_id: jobId, recipient_name: DEMO_RECIPIENT_NAME, recipient_phone: demoPhone, recipient_region: DEMO_REGION, recipient_locale: DEMO_LOCALE, preparation_purpose: DEMO_PURPOSE, question_outline: DEMO_QUESTIONS.join("\n"), request_snapshot: demoSnapshot(), idempotency_key: idempotencyKey, lifecycle_status: "prepared", approval_state: "approved", approved_recipient_phone: demoPhone, approved_at: now.toISOString(), approval_expires_at: expires.toISOString(), provider_call_id: "", provider_status: "", submitted_at: "", completed_at: "", safe_error_category: "", safe_result_summary: "", review_note: "Created for one authorized RepairReady demonstration. No service booking or customer commitment." };
  ...
  if (job.demo_marker !== DEMO_MARKER || attempt.demo_marker !== DEMO_MARKER || job.phone !== demoPhone || attempt.repair_job_id !== jobId || attempt.recipient_phone !== demoPhone || attempt.approved_recipient_phone !== demoPhone || attempt.recipient_region !== DEMO_REGION || attempt.recipient_locale !== DEMO_LOCALE || attempt.approval_state !== "approved" || attempt.lifecycle_status !== "prepared" || attempt.idempotency_key !== idempotencyKey || Date.parse(String(attempt.approval_expires_at)) <= Date.now() || !CANONICAL_PHONE_RE.test(demoPhone)) {
